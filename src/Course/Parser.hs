...
mapParser ::
  ...
  -> Parser a
  -> Parser b
mapParser f (P fun) = P (\s-> case fun s of
...
bindParser f (P parf) =
  P (\s -> case parf s of
        ErrorResult a -> ErrorResult a
        Result input a -> parse (f a) input)
...
list ::
  Parser a
  -> Parser (List a)
list par = list1 par ||| valueParser Nil
...
satisfy ::
  ...
  -> Parser Char
satisfy f =
  bindParser (\s -> if f s then valueParser s else unexpectedCharParser s) character
...
natural ::
  Parser Int
natural =
  flbindParser (list1 digit) (\s -> case read s :: Optional Int of
                                 Empty -> failed
...
sequenceParser (h :. t) =
  ...
  -- foldRight f (valueParser Nil)
  -- where f x acc =
  --         bindParser (\s -> valueParser ()) acc p
...
thisMany ::
  ...
  -> Parser a
  -> Parser (List a)
thisMany n par = sequenceParser $ replicate n par
...
firstNameParser ::
  Parser Chars
firstNameParser =
  flbindParser upper (\a -> flbindParser (list lower) (\as -> valueParser (a :. as) ))
...
surnameParser ::
  Parser Chars
surnameParser =
  flbindParser upper (\a -> flbindParser (thisMany 5 lower)
                              (\as -> flbindParser (list lower)
                                      (\aas -> valueParser (a :. as ++ aas))))
...
phoneParser ::
  Parser Chars
phoneParser =
  flbindParser digit (\a -> flbindParser phoneBodyParser
                              (\as -> flbindParser (is '#')
                                      (\_ -> valueParser (a:.as))))
...
personParser ::
  Parser Person
personParser =
  flbindParser ageParser (\age ->
  spaces1 >>> flbindParser firstNameParser (\name ->
  spaces1 >>> flbindParser surnameParser (\last ->
  spaces1 >>> flbindParser smokerParser (\smoker ->                                            
  spaces1 >>> flbindParser phoneParser (valueParser . Person age name last smoker)))))
...
instance Functor Parser where
  (<$>) ::
    ...
    -> Parser a
    -> Parser b
  (<$>) f p = flbindParser p (valueParser . f)
