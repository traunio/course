...
betweenCharTok ::
  ...
  -> Parser a
  -> Parser a
betweenCharTok c d  = between (charTok c) (charTok d)
...
eof ::
  Parser ()
eof = P (\s -> if s == Nil then Result Nil () else ErrorResult (ExpectedEof s))
...
-- without sequence
-- satisfyAll fs = P (\s -> case s of
--                       Nil -> ErrorResult UnexpectedEof
--                       (x:.xs) -> case and $ fs <*> pure x of
--                         True -> Result xs x
--                         False -> ErrorResult Failed)
