...
instance Functor ListZipper where
  (<$>) f (ListZipper as b cs) = ListZipper (f <$> as) (f b) (f <$> cs)
...
findLeft ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
findLeft f (ListZipper Nil a r) = IsNotZ
findLeft f (ListZipper (l:.ls) a r) = 
    if f l then IsZ (ListZipper ls l (a:.r)) else findLeft f (ListZipper ls l (a:.r))
...
findRight f (ListZipper l a r) = 
    case break f r of
         (_, Nil) -> IsNotZ
         (ls, ri:.rs) -> IsZ (ListZipper (reverse ls ++ a:.l) ri rs)
...
moveLeftLoop (ListZipper Nil a rs) = 
    case reverse rs of
         ...
         (x:.Nil) -> ListZipper (a:.Nil) x Nil
         (x:.xs) -> ListZipper (xs ++ a:.Nil) x Nil         
moveLeftLoop (ListZipper (l:.ls) a rs) = ListZipper ls l (a:.rs)
...
moveRightLoop ::
  ListZipper a
  -> ListZipper a
moveRightLoop (ListZipper ls a Nil) =
    let (x:.xs) = reverse (a:.ls)
    in ListZipper Nil x xs
moveRightLoop (ListZipper ls a (r:.rs)) = ListZipper (a:.ls) r rs
...
moveLeft (ListZipper (l:.ls) a rs) = IsZ (ListZipper ls l (a:.rs))
...
swapLeft (ListZipper (l:.ls) a rs) = IsZ $ ListZipper (a:.ls) l rs
...
dropLefts ::
  ListZipper a
  -> ListZipper a
dropLefts (ListZipper _ a rs) = ListZipper Nil a rs
...
dropRights ::
  ListZipper a
  -> ListZipper a
dropRights (ListZipper ls a _) = ListZipper ls a Nil
...
moveLeftN' n al@(ListZipper l a r)
    | n == 0 = Right al
    | n < 0 = moveRightN' (negate n) al
    | n > length l = Left (length l)
    | otherwise = let (IsZ zi) = moveLeftN n al in Right zi
...
moveRightN' ::
  ...
  -> ListZipper a
  -> Either Int (ListZipper a)
moveRightN' n al@(ListZipper l a r)
    | n == 0 = Right al
    | n < 0 = moveLeftN' (negate n) al
    | n > length r = Left (length r)
    | otherwise = let (IsZ zi) = moveRightN n al in Right zi
...
-- prop> rights (end lz) == Nil
  ...
  ListZipper a
  -> ListZipper a
end al@(ListZipper _ _ r) = 
    let (IsZ da) = moveRightN (length r) al in da
...
start ::
  ListZipper a
  -> ListZipper a
start al@(ListZipper l _ _) = 
    let (IsZ da) = moveLeftN (length l) al in da
